...
let
  ...
  menu = "${pkgs.rofi}/bin/rofi -terminal ${terminal_cmd} -show drun -show-icons";
in
{
  ...
  xsession.windowManager.i3.config = rec {
    ...
    keybindings =
      ...
      in
      {
        ...
        "${mod}+d" = "exec ${nixGLCommandPrefix}${menu}"; # run rofi with nixGL so all program opened inherit it
